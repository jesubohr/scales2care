...
import { scalesPerCategory, scalesDescriptions } from "../utils/scales";
...
export default function Category() {
  ...
  const categoryTitle = (category) => {
    return category
      ...
      .split(" ")
      .map((word) => {
        const first = word.length === 1 ? word : word.charAt(0).toUpperCase();
          ...
          ({ scaleName, scaleId }, index) => {
            return (
              <ScaleButton
                ...
                description={scalesDescriptions[scaleId]}
              />
            );
          }
        )}
